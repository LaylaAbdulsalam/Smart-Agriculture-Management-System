...
const ZoneCard: React.FC<ZoneCardProps> = ({ zone, activeCropDetails, alerts, t, onEdit, onDelete, onViewDetails, onViewHistory }) => {
  ...
  return (
    <div className={`bg-card-light dark:bg-card-dark p-5 rounded-xl shadow-md flex flex-col justify-between border h-full transition-all hover:shadow-lg ${alertCount > 0 ? 'border-red-500/50' : 'border-border-light dark:border-border-dark'}`}>
      ...
      <div className="flex items-center mt-5 pt-4 border-t border-border-light dark:border-border-dark gap-2">
        <div className="flex-1 flex gap-2">
            {hasActiveCrop ? (
                <button onClick={onViewDetails} className="w-full text-center px-3 py-2 bg-primary/10 text-primary font-semibold rounded-lg hover:bg-primary/20 transition-colors text-sm">
                    Manage Current Crop
                </button>
            ) : (
                <button onClick={onViewDetails} className="w-full text-center px-3 py-2 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition-colors text-sm">
                    + Plant Crop
                </button>
            )}
            <button onClick={onViewHistory} className="w-full text-center px-3 py-2 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 font-semibold rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors text-sm">
                View Crop History
            </button>
        </div>
